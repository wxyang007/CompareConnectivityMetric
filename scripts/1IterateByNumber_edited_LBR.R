...
shp <- read_sf('LiberiaPA/wdpa_liberia_neighb230/LiberiaPAFinal230km_mollwd.shp')
shp <- st_zm(shp)
...
# Note: when flip selection (i.e., select non AOI PAs), new NAs are created for California
# therefore I prepared the file in ArcGIS
non_ca_pa <- shp[which(shp$ISO3 != 'LBR'),] # Liberia
...
ca_neartab <- read.csv('LiberiaPA/neartab_liberia_neighb230.csv')
ca_neartab$Rowid_ <- NULL
...
# The below code only needs to be run for Liberia, where there are only a few PAs
# We produce all possible combinations of PAs to select here
...
for (n in 1:nrow(dat)) {  # for Liberia
  ...
  # Option 1: Use function in sf package
  ...
  # Option 2: Use the near table generated from other software
  ...
  r1 <- neartab_corr %>%
    group_by(IN_FID) %>%
    summarise(neighbors = list(unique(NEAR_FID)))
  r2 <- neartab_corr %>%
    group_by(NEAR_FID) %>%
  ...
  # do the calculations for all PAs (both within AOI and its transboundary neighbors)
...
iterspath <- paste0(getwd(), '\\results\\Liberia_100iterations_Feb22_raw.csv')
