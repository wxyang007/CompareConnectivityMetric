...
#  get x percentile
df1 = df
# df1['%dist'] = (df1['dist'] > df['dist'].quantile(0.25)).astype(int)
df1['%dist'] = 0
df1.loc[df1['dist']<=df1['dist'].quantile(0.25), '%dist'] = 1 # either would work
df1.loc[df1['dist']>df1['dist'].quantile(0.75), '%dist'] = 4
len(df1.loc[df1['%dist']==1])

df1['%int_area'] = 0
df1.loc[df1['int_area']<df1['int_area'].quantile(0.25), '%int_area'] = 1
df1.loc[df1['int_area']>df1['int_area'].quantile(0.75), '%int_area'] = 4
len(df1.loc[df1['%int_area']==4])

df1['%prox'] = 0
df1.loc[df1['prox']<df1['prox'].quantile(0.25), '%prox'] = 1
df1.loc[df1['prox']>df1['prox'].quantile(0.75), '%prox'] = 4
len(df1.loc[df1['%prox']==1])

df1['%iic'] = 0
df1.loc[df1['iic']<df1['iic'].quantile(0.25), '%iic'] = 1
df1.loc[df1['iic']>df1['iic'].quantile(0.75), '%iic'] = 4
len(df1.loc[df1['%iic']==1])

df1['%centrality'] = 0
df1.loc[df1['centrality']<df1['centrality'].quantile(0.25), '%centrality'] = 1
df1.loc[df1['centrality']>df1['centrality'].quantile(0.75), '%centrality'] = 4
len(df1.loc[df1['%centrality']==4])

df1['%cohesion'] = 0
df1.loc[df1['cohesion']<df1['cohesion'].quantile(0.25), '%cohesion'] = 1
df1.loc[df1['cohesion']>df1['cohesion'].quantile(0.75), '%cohesion'] = 4
len(df1.loc[df1['%cohesion']==4])

df1.to_csv(r'/Users/wenxinyang/Desktop/ProtConn/data/CA/result/group1_per.csv')


import pandas as pd
df1 = pd.read_csv(r'/Users/wenxinyang/Desktop/ProtConn/data/CA/result/group1_per.csv')
df1['%iic'] = df1['%iic'].astype(int)
df1['%int_area'] = df1['%int_area'].astype(int)
df1['cpib'] = df1['%iic']*df1['%int_area']*(df1['%iic'] - df1['%int_area']+2)
len(df1.loc[df1['cpib']==32,])
len(df1.loc[(df1['%iic'] == 1) & (df1['%int_area'] ==1) & (df1['cpib'] == 2), ])
a = df1.loc[(df1['%iic'] == 1) & (df1['cpib'] !=-4) & (df1['cpib'] != 2) , ]
