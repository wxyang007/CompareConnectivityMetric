...
setwd(wd)
coneforfolder = 'C:/Users/wyang80/Desktop/CONN_MEASURE/data/Colombia/conefor_results'
# IMPORTANT: remember to copy paste the executables into these folders

# read in the PA file
# remember to read in the one with the equal-area projection
shp <- read_sf('ColombiaPA/ColombiaPAFinal10km_SAD_Sim_50m.shp') # Colombia
shp <- st_zm(shp)
summary(shp$OBJECTID)

ca_bound <- read_sf('ColombiaBound.shp')

# filter out only the pas in CA, these are the patches to iterate with
ca_pa <- shp[which(shp$ISO3 == 'COL'), ] # PAs of Colombia

ca_bound_reproj <- st_transform(ca_bound, crs(ca_pa))

non_ca_pa <- shp[which(shp$ISO3 != 'COL'),] # Colombia
...
ca_neartab <- read.csv('ColombiaPA/near_tab_colom_simplify50m0404.csv')
...
# IMPORTANT: execute these lines when rerunning
...
for (n in 1:n_iter) {   
  ...
  OBJECTIDs_to_keep = unique(ca_pa[!ca_pa$OBJECTID %in% OBJECTIDs_to_drop, ]$OBJECTID)
  # Line for rerun
  #OBJECTIDs_to_keep = as.list(as.numeric(strsplit(iters_result[n,]$objectids_to_keep, ",")[[1]]))
  #OBJECTIDs_to_keep <- unique(ca_pa$OBJECTID)   # for computing the base cases
  ...
  # IMPORTANT: remember to check if IDs are the same from the near table file (computed from 230km buffer) 
  # and the shapefile (from 10km buffer)
  ...
  nodedf$AREA_GEO[which(nodedf$ISO3 != "COL")] <- 0 
...
iterspath <- paste0(getwd(), '\\results\\100iterations_April06.csv')
