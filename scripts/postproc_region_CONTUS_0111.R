...
results_folder <- '/Users/wenxinyang/Desktop/ProtConn/results/CONTUS100KM'
d <- 100000
...
table_for_PA_nds <- paste(root_folder, '/CONTUS_attrib_table_PA_mexcanus_nds_sept2021.csv', sep = "") # if generated from script, will be a txt file
table_for_PA_ds <- paste(root_folder, '/CONTUS_attrib_table_PA_mexcanus_final_sept2021.csv', sep = "")
...
table_country_area <- paste(root_folder, '/CONTUS_area.txt', sep = "")

## Output files
outfile_all_results <- paste(results_folder,"/All_values_CONTUS_final_",'test',".txt", sep = "")
outfile_protconn_results <- paste(results_folder,"/ProtConn_results_CONTUS_final_",'test',".txt", sep = "")
...
data_PA_ds <- na.omit(read.csv(table_for_PA_ds) %>% select(ifPA, ifUS, AREA_GEO))
...
data_country_area <- data_country_area %>% select(ifUS, Shape_Area)
