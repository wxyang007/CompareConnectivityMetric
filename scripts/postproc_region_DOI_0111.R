...
root_folder <- '/Users/wenxinyang/Desktop/ProtConn/results'
results_folder <- '/Users/wenxinyang/Desktop/ProtConn/results/DOI100KM'
d <- 100000
...
table_for_PA_nds <- paste(root_folder, '/DOI_attrib_table_PA_mexcanus_nds_aug2021.csv', sep = "") # if generated from script, will be a txt file
table_for_PA_ds <- paste(root_folder, '/DOI_attrib_table_PA_mexcanus_final_aug2021.csv', sep = "")
...
# table_country_area <- 'C:/Users/wyang80/Desktop/ProtConn/results/CONTUS_area.txt'
table_country_area <- paste(root_folder, '/DOI_area.txt', sep = "")

## Output files
outfile_all_results <- paste(results_folder,"/All_values_DOI_final_",'test',".txt", sep = "")
outfile_protconn_results <- paste(results_folder,"/ProtConn_results_DOI_final_",'test',".txt", sep = "")

## we read the tables
data_within_only <- read.table(table_without_trans,header = TRUE)
data_within_only <- data_within_only[data_within_only$Distance == d, ]
data_within_only <- data_within_only[!duplicated(data_within_only), ]
data_within_and_outside <- read.table(table_with_trans,header = TRUE)
data_within_and_outside <- data_within_and_outside[data_within_and_outside$Distance == d, ]
...
data_PA_region_nds <- na.omit(data_PA_region_nds)
...
data_PA_ds <- na.omit(read.csv(table_for_PA_ds) %>% select(ifPA, REG_NUM, AREA_GEO))
...
data_for_BOUND <- data_for_BOUND[data_for_BOUND$Distance == d, ]
data_for_BOUND <- data_for_BOUND[!duplicated(data_for_BOUND),]
...
data_for_BOUND_wr <- data_for_BOUND_wr[data_for_BOUND_wr$Distance == d, ]
data_for_BOUND_wr <- data_for_BOUND_wr[!duplicated(data_for_BOUND_wr), ]
...
data_country_area <- data_country_area %>% select(REG_NUM, Shape_Area)
...
merge_all_data[is.na(merge_all_data)] <- 0
merge_all_data <- merge_all_data[merge_all_data$Prefix != 0, ]
